        ...
        if (canvas != null && data != undefined) {
            ...
            this.socket.on('QuitgameClient', (msg) => {
                ...
                if (msg.gameid === this.gameid && (msg.userId === this.MyData['id'] || msg.userId === this.MyData['id'])) {
                    // axios.post('http://backdend:3001/game/quit/' + this.gameid + '/' + msg.userId,
                    //     {
                    //         map: "none",
                    //     })
                    //     .then(res => {
        ...
        this.intervalId = setInterval(() => {
            if (this.pause) {
                ...
                if (this.time <= 0) {
                    ...
                    axios.post('http://backdend:3001/game/quit/' + this.gameid + '/' + newId,
                        {
                            map: "none",
                        })
                        .then(res => {
    ...
    start() {
        ...
        if (this.paddle_left.score === 10 || this.paddle_right.score === 10) {
            ...
            if (this.paddle_left.score === 10) {
                newId = this.data['user2']['id'];
                axios.post('http://backdend:3001/game/finish/' + this.gameid + '/' + this.data['user2']['id'],
                    {
                        map: "none",
                ...
                newId = this.data['user2']['id'];
                axios.post('http://backdend:3001/game/finish/' + this.gameid + '/' + this.data['user1']['id'],
                    {
                        map: "none",
                        user1_score: this.paddle_right._score + ' ',
                        user2_score: this.paddle_left._score + ' ',
...
const Canvas = (props: any) => {
    ...
    var socket = io('http://backdend:3080');
