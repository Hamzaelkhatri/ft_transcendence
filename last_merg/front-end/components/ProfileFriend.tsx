...
const ProfileFriend = (props) => {
  ...
  const fetchmyprofile = async () => {
    const response = await axios.get(`http://192.168.63.100:3001/profile/users/${props.id}`, {
  ...
  const fetchmyhistory = async () => {
    const response = await axios.get(`http://192.168.63.100:3001/game/history/users/${props.id}`, {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    });
    return response;
  };
