...
const ProfileFriend = (props) => {
  ...
  const fetchmyprofile = async () => {
    const response = await axios.get(process.env.FRONTEND_URL + `:3001/profile/users/${props.id}`, {
  ...
  const fetchmyhistory = async () => {
    const response = await axios.get(process.env.FRONTEND_URL + `:3001/game/history/users/${props.id}`, {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    });
    return response;
  };
